...
export default function (props: any) {
  ...
  const { btnLoading, addOrEdit, getFileList, createId, deleteVideoFile, getClassAll, shareList, shareList2, getFileRecordAll, shares, getShareVideoList } = useAddOrEditModel();

  const open = (type: 'add' | 'edit', row?: any) => {
    setType(type);
    if (type == 'edit') {
      getFileList({ courseVideoId: row.id }).then(list => {
        const arr: any = [];
      ...
      createId().then(res => {
        if (res?.resultCode == successCode) {
          addId.current = res.data?.id;
          setExtrData({ courseVideoId: res.data?.id, fileNumber: 1 });
        } else {
  ...
  const onOk = async () => {
    ...
    if (valid) {
      ...
      addOrEdit({ id: addId.current, title, content, courseId: props.courseId, ...extParams }, type, () => {
        props?.reload();
        setVisible(false);
        deleFiles();
      });
  ...
  // 视频上传相关;
  const uploadSucess = () => {
    // getFileList({ courseVideoId: addId.current }).then(list => {
    //   const item = list[0] || {};
    //   setNfile({ id: item.id });
    //   setOfileList(list);
    // });
  ...
  const uploadProps = {
    action: `${baseUrl}/services/course/video/upload`,
    data: extrData,
    fileList,
