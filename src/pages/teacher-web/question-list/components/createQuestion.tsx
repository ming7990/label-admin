...
import { qualityQuestionLis } from "@/pages/teacher-web/knowledge-base-manage/model/api"
...
const ModelComponent: React.FC<any> = (props: any) => {
  const { courseId, cref, questionDetail, questionInsert, questionUpdate, reload } = props;
  ...
  // 添加状态存储题目列表  
  const [qualityQuestionList, setQualityQuestionList] = useState<any[]>([]);
  ...
  const titleSource = Form.useWatch('titleSource', form);  // 监听题目来源   
  ...
  const open = async (type: string, row: any) => {
    ...
    setFormType(type);
    if (type == 'edit') {
      await questionDetail({ id: row?.id }).then(async (res: any) => {
        setTableInfo(res?.data || row);
        const { id, question, questionType, questionNumber, optionList = [], similarity, keyPointList = [], titleSource = "" } = res?.data || {};
        // 如果是品质知识库来源，需要先获取题目列表  
        if (titleSource === 1) {
          await fetchQualityList();
        }
        setId(id);
        const list: string[] = [];
        let answer: any = '';
        ...
        form.setFieldsValue({
          titleSource,
          question,
          questionType, questionNumber: questionNumber.slice(1), // 去掉前面的字母
          optionList: list,
          answer,
          similarity,
  ...
  const onOk = async () => {
    ...
    if (valid) {
      ...
      if (formType == 'add') {
        ...
        const res = await questionUpdate({ id, ...params });
        if (res) {
          onCancel();
          reload();
        }
  ...
  const fetchQualityList = async () => {
    try {
      const res = await qualityQuestionLis();
      if (res?.data) {
        setQualityQuestionList(res.data?.list || []);
      }
    } catch (error) {
  ...
  // 在题目来源改变时获取题目列表  
  const handleTitleSourceChange = (value: number) => {
    console.log(value)
    form.setFieldsValue({ question: undefined });
    if (value === 1) {
      // 当选择品质知识库时，调用接口获取题目列表 
      fetchQualityList()
    }
  }
  ...
  const handleQuestionSelect = (value: string) => {
    const selectedQuestion = qualityQuestionList.find(item => item.id === value);
    if (selectedQuestion) {
      form.setFieldsValue({
        question: selectedQuestion.knowledgePoints
      });
    }
  }
    ...
    >
      <Form form={form} layout="horizontal" {...formItemLayout}>
        ...
        >
          ...
          >
            {[{ label: '手工编辑', value: 0 }, { label: '品质知识库题库', value: 1 }]?.map((item: any) => (
        ...
        <Condition r-if={[QuestionType.Radio, QuestionType.Multi, QuestionType.Judge, QuestionType.Fill, QuestionType.ShortAnswer, QuestionType.Essay].includes(questionType)}>
          <Form.Item name="question" label="题干" rules={[{ required: true, message: form.getFieldValue('titleSource') === 1 ? '请选择题目' : '请输入题目内容' }]}>
            {titleSource === 1 ? (
              <Select
                placeholder="请选择题目"
              ...
              >
                {qualityQuestionList.map((item: any) => (
