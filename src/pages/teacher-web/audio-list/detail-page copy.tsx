  ...
  const finalSubmit = () => {
    ...
    if (taskId && nodeId) {
      submitInfo({
        ...
        studyId: _studyId,
        courseId,
        process: videoElement.current.currentTime,
        taskNodeId: nodeId,
        titleId: info.id,
        videoId: playVideoId,
        mark: mark
  ...
  return (
    <div className={style['content-detail-box']}>
      ...
      <div style={{ display: 'flex' }}>
        <p>录音备注{_studyId}</p>
        <TextArea style={{ margin: '0 10px', flex: 1 }} onChange={(e) => setMark(e.target.value)} />
        <Button type="primary" onClick={finalSubmit}>提交</Button>
      </div>
    </div >
  );
}
